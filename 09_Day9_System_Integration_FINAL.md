...
|---|---|---|---|---|---|---|---|
| **R-1** | **Payload Mass** | 20,000 kg to SSO | **20,000 kg** | **12,000 kg** (Restricted) | Day 7 §7 (Table 2); Day 8 §2.3 (Table 2) | 3-DOF Numerical Ascent Integration (`day7_sim/sim.py`) | **CLOSED.** Path A meets cargo target; Path B trades cargo to retain GLOM limit. |
...
| **R-3** | **Reusability Goal** | First-Stage VTVL V&V | **First Stage VTVL** (Hybrid Catch) | **First Stage VTVL** (Hybrid Catch) | Day 6 §3.3; Day 7 §5; Day 8 §4 | 2-DOF Descent integration (`day7_sim/recovery_sim.py`) | **CLOSED.** Closed recovery corridor via **34.5 t reserve** and ship at **489 km**. |
| **R-4** | **Marginal Launch Cost** | < $30M per launch | **$37.58M** (Exceeds) | **$24.41M** (Satisfies) | Day 8 §2.3 (Table 2) | Parametric Lifecycle Cost Model (`08_Day8_FINAL.md` Eq. 2.2) | **CLOSED.** Path B meets target; Path A trades cost limit to deliver 20 t cargo. |
...
*   **Our Defense:** "Prof. Xu, this is a strategic trade between absolute launch cost and specific transport efficiency. Path B satisfies the absolute cost limit ($24.41M per launch) but restricts the payload to 12 t, yielding a specific cost of **$2,033/kg**. Path A exceeds the launch cost limit ($37.58M) but delivers the full 20 t cargo, achieving a superior specific cost of **$1,879/kg** (an 8.2% efficiency gain). For megaconstellation customers launching 100+ tonnes of satellites annually, Path A is the economically optimal choice because it minimizes the total program cost. Furthermore, Path A's 12-engine Octaweb-style layout provides robust engine-out capability during ascent, enhancing flight reliability."
...
*   **Our Defense:** "Yingjie, because our minimum thrust-to-weight ratio near touchdown is $\approx 1.8$ (338 kN thrust / 185 kN booster mass at burnout), a stable hover is physically impossible. This is why our recovery concept is a committed **hover-slam (or retro-thrust landing)**. The landing computer solves a boundary-value problem in real-time, executing an adaptive ignition bisection. The engine ignites late (between 1,500 m and 2,000 m) and burns at a high throttle, and the velocity is nulled to $\le 2$ m/s exactly at the 15 m capture height. The engine shuts down immediately upon cable engagement. This matches the operational landing profile of Falcon 9, and our Day 7 Monte-Carlo simulation proves that this logic closes with a 95% success rate under severe wind and atmospheric dispersions."
